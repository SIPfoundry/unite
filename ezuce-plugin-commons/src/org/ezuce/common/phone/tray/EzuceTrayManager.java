...
import org.jivesoftware.sparkimpl.plugin.systray.ChatMessageHandlerImpl;
 ...
 * @version 0.1
 */
public class EzuceTrayManager implements NativeHandler, ChatManagerListener,
		ChatStateListener {
	...
	private static final EzuceTrayManager trayManager = new EzuceTrayManager();

	public static EzuceTrayManager getInstance() {
		return trayManager;
	}

	public EzuceTrayManager() {
	}
	...
	public void update(Presence presence) {
		...
		cachedPresence = EzucePresenceManager.copy(presence);
		buildIcon();
	}
	...
	public void initialize() {
		if (!isInitialized && SystemTray.isSupported()) {
			...
			try {
				...
				cachedPresence = EzucePresenceManager.copy(SparkManager
						.getWorkspace().getStatusBar().getPresence());
				isInitialized = true;
				buildIcon();
	...
	private AbstractAction createCustomStatusItemAction(
		...
		AbstractAction customAction = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Presence presence = EzucePresenceManager.copy(oldPresence);
	...
	private void buildIcon(Image[] icons) {
		...
		if (icons == null || icons.length < 4) {
			System.out
					.println("EzuceTrayManager#buildIcons() icons don't exist");
			return;
		}
		...
		SparkManager.getConnection().addConnectionListener(
				new ConnectionListener() {
					...
					public void reconnectionSuccessful() {
						buildIcon(PresenceIconProducer.getConnectingIcons());
						if (PhoneManager.getInstance().isUserRegistered()) {
							getInstance().updateRegisteredAsPhone(true);
						}
					}
